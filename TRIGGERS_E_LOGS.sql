...
CREATE OR REPLACE PROCEDURE backupUsuario()

CREATE OR REPLACE TRIGGER backupUsuario
AFTER UPDATE OR DELETE ON usuarios
FOR EACH ROW
BEGIN
	IF UPDATING THEN
		INSERT INTO BACKUP_USUARIOS
			VALUES (:OLD.id, :OLD.nome, :OLD.idade, 'UPDATE', SYSTIMESTAMP);
	ELSE 
		INSERT INTO BACKUP_USUARIOS
			VALUES (:OLD.id, :OLD.nome, :OLD.idade, 'DELETE', SYSTIMESTAMP);
	END IF;
END;
